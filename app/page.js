...
export default function HomePage() {
  ...
  const latestDraw =
    games.find(
      (g) =>
        g.today_number && g.today_number !== "XX" && g.today_number !== "--",
    ) || games[0];
      ...
      {latestDraw && (
        <div className="lrs">
          ...
          <span className="lrs-num">
            {latestDraw.today_number === "XX" ||
            latestDraw.today_number === "--"
              ? "??"
              : latestDraw.today_number}
          </span>
          <a
      ...
      <main className="wrap">
        ...
        <div className="wa-banner">
          <div className="wa-banner-text">
            <span className="wa-banner-title">
              👑 सीधा खाईवाल से गेम पास करवाएं &bull; 100% ईमानदार और सुपरफास्ट
              पेमेंट
            </span>
            <span className="wa-banner-sub">
              जोड़ी और हरूफ गेम सीधे WhatsApp पर प्राप्त करें &bull; नंबर:{" "}
              {WHATSAPP_NUMBER}
            </span>
          </div>
          <a
        ...
        <div className="section-head">
          ...
          >
            <span
              className="lrs-dot"
              style={{ background: syncing ? "var(--amber)" : "#43e660" }}
            />
            ...
            heroes.map((g) => {
              ...
              return (
                <div key={g.code} className="hero-card" id={`hero-${g.code}`}>
                  ...
                  <span
                    className={`hero-number ${isPending ? "pending-hero" : ""}`}
                  >
        ...
        <div className="section-head" style={{ marginTop: 40 }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            <span className="section-meta" id="yesterday-label">
              ↩ {fmt(yesterdayDate)}
            </span>
            <span
              className="section-meta"
                ...
                <div className="results-grid">
                  {catGames.map((g) => {
                    const isPending =
                      ...
                      g.today_number === "XX" ||
                      g.today_number === "--";
                    const todayCls = isPending
                      ? "pending"
                      : `today${g.is_highlight ? " is-highlight-num" : ""}`;
                    ...
                    return (
                      <div
                        key={g.code}
                        className={`game-card ${isHighlight}`}
                        id={`card-${g.code}`}
                      >
                        ...
                        <div className="numbers-wrapper">
                          <div className="num-box">
                            ...
                            <span className="num-badge yesterday">
                              {g.yesterday_number || "—"}
                            </span>
                          </div>
                          <div className="num-box">
        ...
        <div className="section-head" style={{ marginTop: 48 }}>
          ...
          <span className="section-meta" id="chart-subtitle">
            {chartData
              ? `${chartData.days_in_month || 31} days · Combined Record Archive`
              : "Archive"}
          </span>
        </div>
        ...
        <div className="chart-wrapper">
          <table
            ...
            id="monthly-table"
            aria-label="Monthly result chart"
          >
            <thead>
              <tr>
              ...
              {chartData?.rows?.map((r) => {
                ...
                return (
                  <tr key={r.day}>
                    ...
                    <td className={`num-col ${cell(r.DS)}`}>
                      {r.DS === "XX" && isToday ? <SpinnerIcon /> : r.DS || "—"}
                    </td>
                    <td className={`num-col ${cell(r.FB)}`}>
                      {r.FB === "XX" && isToday ? <SpinnerIcon /> : r.FB || "—"}
                    </td>
                    <td className={`num-col ${cell(r.GB)}`}>
                      {r.GB === "XX" && isToday ? <SpinnerIcon /> : r.GB || "—"}
                    </td>
                    <td className={`num-col ${cell(r.GL)}`}>
          ...
          <div className="month-nav">
            ...
            <span
              ...
              id="nav-current-month"
            >
              {chartData
                ? `${MONTH_NAMES[mIdx]?.substring(0, 3).toUpperCase()} ${chartData.year}`
                : ""}
            </span>
            <button
              className="nav-btn"
